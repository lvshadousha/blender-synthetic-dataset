...
如何运行（Blender UI 内执行）
使用 Blender 打开工程文件：hhhexperiment_model.blend
打开 Scripting（脚本） 工作区
在文本编辑器（Text Editor）中找到脚本：渲染脚本.py
（强烈建议）先把渲染数量设置小一些进行测试
点击 Run Script（运行脚本） 执行
运行时注意事项
...
脚本执行期间会占用主线程/渲染资源，导致 UI 无响应，这是你当前脚本运行方式���正常表现。等任务完成后 UI 会恢复。
